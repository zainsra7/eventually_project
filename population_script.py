...
def add_event(event):
    ...
    e = Event.objects.get_or_create(title=event["title"], host=up, date=event_date.date(), time=event['time'], post_code=event['post_code'], description=event['description'],image=event['image'], capacity=event['capacity'], address=event['address'], fb_page=event['fb_page'])[0]
    e.save()
    return e
